...
Class Master extends DBConnection {
	...
	function save_supplier(){
		...
		$check = $this->conn->query("SELECT * FROM `cust_list` where `name` = '{$name}' ".(!empty($id) ? " and id != {$id} " : "")." ")->num_rows;
		if($this->capture_err())
		...
		if(empty($id)){
			$sql = "INSERT INTO `cust_list` set {$data} ";
			$save = $this->conn->query($sql);
		}else{
			$sql = "UPDATE `cust_list` set {$data} where id = '{$id}' ";
			$save = $this->conn->query($sql);
		}
		if($save){
			$resp['status'] = 'success';
	...
	function save_rooms(){
		...
		$check = $this->conn->query("SELECT * FROM `rooms` where `room_co` = '{$room_co}' ".(!empty($id) ? " and id != {$id} " : "")." ")->num_rows;
		if($this->capture_err())
			return $this->capture_err();
		if($check > 0){
			$resp['status'] = 'failed';
	...
	function delete_supplier(){
		extract($_POST);
		$del = $this->conn->query("DELETE FROM `cust_list` where id = '{$id}'");
		if($del){
			$resp['status'] = 'success';
